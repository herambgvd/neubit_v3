...
export default function AdminLoginPage() {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- this IS the mounted flag the comment above describes; it exists precisely to keep the server render and the first client paint identical.
  ...
  function finish(data: LoginResult) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data?.mfa_required) {
        // First factor OK — switch to the code step. A challenge without a token
        // cannot be answered, so surface it rather than silently re-showing the
        // password form (which would look like the password was wrong).
        if (!data.mfa_token) throw new Error("The server did not issue a 2FA challenge token.");
