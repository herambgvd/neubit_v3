  ...
  return (
    <div>
      ...
      <form onSubmit={applyFilter} className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          type="submit"
          className="inline-flex items-center gap-2 rounded-lg border border-card-border bg-card px-3.5 py-2 text-sm font-medium text-foreground transition hover:border-muted hover:text-foreground"
        >
          Apply
        </button>
        {tenantId && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-muted transition hover:text-foreground"
          >
            Clear
          </button>
        )}
      ...
      {tenantId && (
        <div className="mb-4 inline-flex items-center gap-2 rounded-lg border border-cyan-400/20 bg-cyan-500/10 px-3 py-1.5 text-xs text-cyan-600 dark:text-cyan-300">
      ...
      <div className="overflow-hidden rounded-2xl border border-card-border bg-card">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {isError && (
              <tr>
                <td colSpan={4} className="px-5 py-10 text-center text-sm text-red-600 dark:text-red-300">
            ...
            {!isLoading &&
              !isError &&
              items.map((row) => (
                <tr key={row.id} className="border-b border-card-border last:border-0 transition hover:bg-hover">
                  <td className="whitespace-nowrap px-5 py-3.5 text-muted tabular-nums">{fmtTs(row.ts)}</td>
                  ...
                  <td className="px-5 py-3.5 text-foreground">
                    {row.target_type || row.target_id ? (
                      <span>
                        <span className="text-muted">{row.target_type || "—"}</span>
                        {row.target_id ? <span className="font-mono text-xs text-muted"> · {row.target_id}</span> : null}
                      </span>
                    ) : (
    ...
    <tr key={i} className="border-b border-card-border last:border-0">
      {Array.from({ length: 4 }).map((__, j) => (
        <td key={j} className="px-5 py-4">
          <div className="h-3.5 w-full max-w-[140px] animate-pulse rounded bg-hover" />
        </td>
      ))}
    </tr>
  ));
