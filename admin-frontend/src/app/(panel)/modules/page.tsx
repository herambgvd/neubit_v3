...
export default function ModulesPage() {
  ...
  // Memoized so the filter below actually memoizes (a new [] each render would
  // change its dependency every time).
  ...
  const del = useMutation({
    mutationFn: (key: string) => adminApi.deleteModule(key),
    onSuccess: () => {
...
type ModuleValues = yup.InferType<typeof moduleSchema>;
...
function ModuleModal({
  ...
  const save = useMutation({
    mutationFn: (values: ModuleValues) => {
      ...
      if (isEdit && module) return adminApi.updateModule(module.key, body);
