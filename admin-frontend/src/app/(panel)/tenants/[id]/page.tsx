...
export default function TenantDetailPage() {
  ...
  const setStatus = useMutation({
    mutationFn: (suspend: boolean) =>
      suspend ? adminApi.suspendTenant(id) : adminApi.reactivateTenant(id),
  ...
  // Settled, not an error, but no row came back — the same dead end as a 404, so
  // say so rather than rendering a page of blanks.
  if (!t) {
    return (
      <Card className="border-danger/20 bg-danger/5 p-5 text-sm text-danger">
        Tenant not found
...
function LicenseCard({ tenant, onSaved }: { tenant: Tenant; onSaved: () => void }) {
  ...
  const catalog = useMemo(() => modulesQ.data ?? [], [modulesQ.data]);
  ...
  const toggleFeature = (key: string, on: boolean) => setFeatures((f) => ({ ...f, [key]: on }));
  ...
  const save = useMutation({
    ...
    onError: (e: Error) => toast.error(apiError(e, e.message || "Could not save license")),
  });
...
/** One row of the console-nav map. A leaf either names a module or is `core`
 *  (always on); a branch carries `children`. */
interface TreeItem {
  label: string;
  sub?: string;
  module?: string;
...
function TreeRow({
  ...
  const on = item.core ? true : !!(item.module && features[item.module]);
  return (
    <div style={pad} className="flex items-center justify-between gap-3 px-3 py-2.5">
      ...
      <Switch
        checked={on}
        disabled={!!item.core}
        onCheckedChange={(v) => !item.core && item.module && toggle(item.module, v)}
      />
    </div>
  );
}
...
function treeModuleKeys(): Set<string> {
  const keys = new Set<string>();
  const walk = (items: TreeItem[]): void =>
    items.forEach((it) => {
...
function SubscriptionCard({
  ...
  const voidInv = useMutation({
    mutationFn: (id: string) => adminApi.voidInvoice(id),
...
/** The chip colours the timeline uses. "default" is the neutral one. */
type TimelineTone = "success" | "warning" | "danger" | "accent" | "default";
...
function metaFor(action: string): ActionMeta {
  return (
    ACTION_META[action] ?? {
      label: humanizeKey(action).replaceAll(".", " "),
      tone: "default",
      Icon: Activity,
    }
  );
...
function ActivityCard({ tenantId }: { tenantId: string }) {
  const q = useInfiniteQuery({
    ...
    initialPageParam: 1,
    // The envelope carries total + page_size (there is no `pages` field), so the
    // last page is derived. Reading a non-existent `pages` made this always
    // undefined, which is why "Load more" never appeared.
    getNextPageParam: (last) => {
      const pages = Math.max(1, Math.ceil((last.total || 0) / (last.page_size || 1)));
      return last.page < pages ? last.page + 1 : undefined;
    },
  });
...
function AdminsCard({
  ...
  const del = useMutation({
    mutationFn: (userId: string) => adminApi.deleteTenantAdmin(tenantId, userId),
    onSuccess: () => {
