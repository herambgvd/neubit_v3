...
// Settings values arrive as an untyped JSON map (the catalog is data, so the
// backend cannot type each key). These narrow one value for a form control.
...
// Loader. Each form below is a separate component mounted only once its values
// exist, so it seeds state from props instead of syncing in an effect — which
// also stops a background refetch overwriting what the operator is typing.
function SettingsCard() {
  ...
  return (
    <SettingsSection
      ...
      subtitle="Defaults applied across all tenants."
    >
      {isError ? (
        ...
        <SettingsForm initial={data.values ?? {}} />
      )}
    </SettingsSection>
  );
}
...
function SettingsForm({ initial }: { initial: Record<string, unknown> }) {
  ...
  const [form, setForm] = useState<SettingsValues>(() => ({
    ...
    support_email: asText(initial.support_email),
    allow_avatar_uploads: asBool(initial.allow_avatar_uploads),
    allow_signups: asBool(initial.allow_signups),
  }));
  ...
  function set<K extends keyof SettingsValues>(k: K) {
    return (v: SettingsValues[K]) => setForm((f) => ({ ...f, [k]: v }));
  }
...
interface MapsValues {
  google_maps_enabled: boolean;
  google_maps_api_key: string;
  /** Kept as strings so the number inputs stay controlled while being typed. */
  google_maps_default_lat: string;
  google_maps_default_lng: string;
  google_maps_default_zoom: string;
}
...
function MapsCard() {
  ...
  return (
    <SettingsSection
      icon={MapIcon}
      title="Google Maps"
      subtitle="API key + default map centre for the Sites Map."
    >
      {isError ? (
        <p className="text-sm text-danger">{apiError(error, "Failed to load settings")}</p>
      ) : isLoading || !data ? (
...
function MapsForm({ initial }: { initial: Record<string, unknown> }) {
  ...
  const [form, setForm] = useState<MapsValues>(() => ({
    ...
    google_maps_api_key: asText(initial.google_maps_api_key),
    google_maps_default_lat: asText(initial.google_maps_default_lat),
    google_maps_default_lng: asText(initial.google_maps_default_lng),
    google_maps_default_zoom: asText(initial.google_maps_default_zoom),
  }));
  ...
  function set<K extends keyof MapsValues>(k: K) {
    return (v: MapsValues[K]) => setForm((f) => ({ ...f, [k]: v }));
  }
...
function BrandingForm({ initial }: { initial: Branding }) {
  ...
  const uploadLogo = useMutation({
    mutationFn: (file: File) => adminApi.uploadPlatformLogo(file),
    onSuccess: (res) => {
  ...
  function set<K extends keyof BrandingValues>(k: K) {
    return (v: BrandingValues[K]) => setForm((f) => ({ ...f, [k]: v }));
  }
