...
import type { DbImportResult } from "@/lib/types";
import {
  Button,
  Card,
  CardContent,
...
function ImportCard() {
  ...
  const importDb = useMutation({
    // Guarded by `disabled={!file}` on the button and by the confirm dialog,
    // which only opens once a file is chosen.
    mutationFn: () => {
      if (!file) throw new Error("Choose a .sql backup file first");
      return adminApi.importDatabase(file);
    },
    onSuccess: (res) => {
      setResult(res);
