...
import type { RecoveryCodes as RecoveryCodesOut } from "@/lib/types";
import {
  Badge,
  Button,
  Card,
        ...
        {isLoading ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <p className="text-sm text-muted">
              {data?.recovery_codes_remaining ?? 0} recovery code
      ...
      <DialogContent className="max-w-lg">
        {recovery ? (
          ...
          <CodeConfirmBody
            ...
            onSuccess={(res) => setRecovery((res as RecoveryCodesOut)?.recovery_codes || [])}
          />
        )}
      </DialogContent>
    </Dialog>
...
/** What a code-confirmed action needs. The result shape varies by action, so it
 *  is `unknown` and each caller narrows it. */
...
type PasswordValues = yup.InferType<typeof pwSchema>;
...
function ChangePasswordCard() {
  ...
  const save = useMutation({
    mutationFn: (v: PasswordValues) => adminApi.changePassword(v.current_password, v.new_password),
    onSuccess: () => {
      form.reset();
...
function deviceName(ua: string | null | undefined): string {
