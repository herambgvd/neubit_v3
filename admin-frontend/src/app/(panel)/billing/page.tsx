...
function PlansTab() {
  ...
  // A plan row to edit, or `{}` to create a new one.
  ...
  return (
    <div className="space-y-4">
      ...
      {editing && (
        <PlanDialog
          plan={editing.key ? (editing as Plan) : null}
...
function PlanDialog({
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      if (plan) return adminApi.updatePlan(plan.key, body);
      return adminApi.createPlan({ ...body, key: key.trim(), features: {} });
    },
    onSuccess: () => {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader
          title={isEdit && plan ? `Edit ${plan.name}` : "New plan"}
    ...
    <div className="space-y-4">
      ...
      {items.length > 0 && (
        <div className="flex items-center justify-between text-xs text-muted">
          <span>{data?.total ?? items.length} invoices</span>
          <div className="flex items-center gap-2">
