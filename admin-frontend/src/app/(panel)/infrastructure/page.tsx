...
/** The lifecycle verbs this page can run against a container. */
...
/** Utilisation tones. `accent` is the neutral/"fine" end of the scale. */
type UtilTone = "accent" | "warning" | "danger";

/** Rolling per-container series driving the sparklines. */
interface Series {
  cpu: number[];
  mem: number[];
}
...
// ── helpers ────────────────────────────────────────────────────────────────
function uptime(createdAt: string | null | undefined): string {
  if (!createdAt) return "—";
...
function fmtMem(used: number | null | undefined, limit: number | null | undefined): string {
  if (used == null && limit == null) return "—";
  return `${fmtMB(used)} / ${fmtMB(limit)}`;
}

function memPct(c: Container): number {
...
function utilTone(pct: number): UtilTone {
  if (pct >= CRIT) return "danger";
  if (pct >= WARN) return "warning";
  return "accent";
}
const TONE_VAR: Record<UtilTone, string> = {
  danger: "var(--danger)",
  warning: "var(--warning)",
  accent: "var(--accent)",
};
const TONE_TEXT: Record<UtilTone, string> = {
  danger: "text-danger",
  warning: "text-warning",
  accent: "text-foreground",
};

/** A util tone as the StatCard understands it — its neutral is "muted". */
const statTone = (t: UtilTone): StatTone => (t === "accent" ? "muted" : t);

// state + health → kit Badge tone.
function stateTone(container: Container): BadgeTone {
...
export default function InfrastructurePage() {
  ...
  // Container name whose log drawer is open.
  const [selected, setSelected] = useState<string | null>(null);
  // Name → the lifecycle action currently in flight for that container.
  const [busy, setBusy] = useState<Record<string, LifecycleAction>>({});
  const [stopping, setStopping] = useState<Container | null>(null);
  // Name → its rolling cpu/mem series.
  ...
  // Memoized: the derived lists below depend on it, and a fresh [] each render
  // would make those useMemo calls no-ops.
  ...
  useEffect(() => {
    if (!data) return;
    const rows = data;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- a rolling history accumulates ACROSS polls; it cannot be derived from the current snapshot, which is all render has.
    setHistory((prev) => {
  ...
  const memTotal =
    host.data?.mem_total_mb ?? Math.max(0, ...containers.map((c) => Number(c.mem_limit_mb) || 0));
  const memUtil = memTotal > 0 ? (memUsed / memTotal) * 100 : 0;

  const invalidate = () => qc.invalidateQueries({ queryKey: ["infra", "containers"] });

  async function runAction(name: string, action: LifecycleAction) {
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          tone={statTone(utilTone(cpuUtil))}
        />
        <StatCard
          label="Memory in use"
          ...
          tone={statTone(utilTone(memUtil))}
        />
        <StatCard
          label="Auto-refresh"
          value="4s"
...
function ScaleControl() {
  ...
  // Held as a string so the number input stays controlled mid-edit (an empty
  // field is a legal intermediate state); coerced when the form is submitted.
