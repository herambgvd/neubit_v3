    ...
    <div>
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Tenants, users, and license health at a glance.</p>
        </div>
        <Link
          href="/tenants"
              ...
              <ul className="mt-3 divide-y divide-white/5">
                {licenseTrouble.map((t) => (
                  <li key={t.id} className="flex items-center justify-between py-2.5">
                    <Link href={`/tenants/${t.id}`} className="text-sm text-white hover:text-cyan-300">
                      {t.name}
                    </Link>
                    <span
                      className={
