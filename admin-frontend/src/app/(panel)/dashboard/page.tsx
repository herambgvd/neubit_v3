...
import { AreaTrend, BarList, ChartCard, DonutChart, type ChartDatum } from "@/components/charts";

// Cumulative tenant count at the end of each of the last `months` months.
function growthSeries(tenants: Tenant[], months = 6): ChartDatum[] {
  const now = new Date();
...
export default function DashboardPage() {
  ...
  // Memoized because every derived chart below lists it as a dependency — a new
  // array each render would make those useMemo calls no-ops.
  const tenants = useMemo(() => pagedItems(data), [data]);
  ...
  const license = useMemo<ChartDatum[]>(
    () => [
  ...
  const plans = useMemo<ChartDatum[]>(() => {
  ...
  const topTenants = useMemo<ChartDatum[]>(
    () =>
      [...tenants]
