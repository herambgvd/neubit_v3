...
class TenantService:
    ...
    async def delete_tenant(self, tenant_id: uuid.UUID) -> tuple[Tenant, dict[str, int]]:
        ...
        records in the audit entry as evidence the erasure happened.

        The order is load-bearing: ``erase_tenant_data`` must run first, in this
        same transaction, because four of core's tables carry ON DELETE SET NULL to
        ``tenants`` and a NULL tenant_id marks a platform default — the departed
        tenant's branding and stored SMTP credentials would be promoted to the
        defaults every other tenant inherits. See app/tenancy/erasure.py.
        """
        ...
        # One commit for the erase and the delete, so a failure above — including
        # the classification check refusing an unclassified table — leaves the
        # tenant intact rather than half erased.
        await self.db.commit()
