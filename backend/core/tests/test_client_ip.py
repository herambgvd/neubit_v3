...
Everything that needs the caller's address goes through one resolver, `client_ip`.
It trusts `X-Forwarded-For` only from a configured proxy and takes the rightmost
untrusted hop, because a trusted proxy appends the peer it saw rather than replacing
what the client sent.

The two ways to get this wrong: `request.client.host` behind Traefik is the gateway,
so every caller shares one bucket; and believing the leftmost forwarded hop lets a
...
def test_an_untrusted_peer_cannot_forge_its_address(trusting_nobody):
    """The default: nothing is trusted, so the header is ignored. Correct for a core
    exposed directly, and what makes the setting safe to ship empty."""
...
def test_a_client_supplied_hop_is_not_believed_over_the_one_the_proxy_saw(trusting):
    """A client sends its own X-Forwarded-For and the proxy appends the peer it saw,
    so the leftmost hop is whatever the caller wrote. Take the rightmost untrusted
    one."""
    forged = "9.9.9.9"
    real = "203.0.113.9"
...
def test_garbage_in_the_header_does_not_become_a_bucket_key(trusting):
    """A non-address hop falls back to the peer. The value becomes a rate-limit
    bucket key, so keys built from arbitrary text would grow the store without
    bound."""
...
def test_an_unparseable_cidr_does_not_silently_trust_everything(monkeypatch):
    """A typo in the setting must degrade to "trust nothing", not "trust all"."""
...
def test_the_rate_limiter_and_the_session_recorder_agree():
    """These each used to resolve the address themselves, differently. Asserting they
    share one resolver stops them drifting apart again."""
    import inspect
    ...
    def code(fn: object) -> str:
        """Source with the docstring removed — the docstrings quote the old
        expressions, so raw source would match the explanation of the bug."""
        import ast
        import textwrap
...
#
# The middleware is on the path of every request, so it needs `client_ip` as much as
# the two rate-limit dependencies do. All three are asserted, because fixing two of
# the three is the easy mistake.
...
def test_probes_are_exempt_by_exact_path_not_by_prefix():
    """Exact match, not `startswith`: a prefix match also exempts `/health-bypass`
    and `/metrics-anything`."""
...
def test_files_and_docs_are_no_longer_uncapped():
    """`/files` is the highest-bandwidth route in the product, and `/docs` and
    `/openapi.json` are what an attacker enumerates first. A bigger budget covers
    the legitimate use, so neither needs an exemption."""
    from app.core.api import GlobalRateLimitMiddleware as M
