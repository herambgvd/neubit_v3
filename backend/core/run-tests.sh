...
# the bottom. A three-minute suite that prints nothing until it finishes reads as
# a hang, so streaming is not negotiable here.
RUN_LOG="$(mktemp -t neubit-core-tests)"
