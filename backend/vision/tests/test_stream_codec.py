...
# The SERVICE layer of the stream-codec policy is gone: the VMS no longer pushes an
# encoder setting to a camera, because writing the encoder is config authorship over a
# device the recorder owns. The DRIVER layer above still has its tests — the brand
# request-building is real code with real edge cases — and it is exercised directly.
