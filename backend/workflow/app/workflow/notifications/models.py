...
One feature, because a single delivery walks all four tables: a template renders
the message, a channel supplies the provider config, an outbox row holds it until
the dispatch job drains it, and a push resolves the recipient's device tokens.
...
class Notification(Base, _TenantTimestamped):
    ...
    # pending | claimed | sent | failed. ``claimed`` means exactly one worker owns
    # this row now: the claim is taken under FOR UPDATE SKIP LOCKED and committed,
    # so the state, not the lock, keeps a second replica off it during a send.
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, server_default=text("'pending'"), index=True
    )
    # When the claim was taken and by which worker. ``claimed_at`` is the lease
    # clock: older than VE_WORKFLOW_NOTIFY_CLAIM_LEASE means the claimer died and
    # the row goes back to pending. Both are NULL in every non-claimed state.
    # Separate from last_attempt_at, which answers a different question.
...
class DeviceToken(Base, _TenantTimestamped):
    ...
    The push connector resolves a user's enabled tokens at dispatch time. Isolation
    is the ``(tenant_id, user_id)`` filter. A token the provider reports invalid is
    disabled rather than deleted, so the audit trail survives.

    ``platform`` is ``fcm`` or ``apns``; ``token`` is the opaque provider
    registration token. ``(tenant_id, platform, token)`` is unique, so re-registering
    a device is an upsert.
    """
