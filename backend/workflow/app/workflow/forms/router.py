...
async def _form_svc(db: Annotated[AsyncSession, Depends(get_db)], scope: Annotated[Scope, Depends(get_scope)]):
...
@form_router.get("", response_model=list[S.FormPublic],
                 dependencies=[Depends(require_permission(perms.FORM_READ))])
...
async def create_form(body: S.CreateFormRequest, svc: Annotated[FormService, Depends(_form_svc)],
                      actor: Annotated[Principal, Depends(require_permission(perms.FORM_CREATE))]):
...
async def update_form(form_id: str, body: S.UpdateFormRequest, svc: Annotated[FormService, Depends(_form_svc)],
                      actor: Annotated[Principal, Depends(require_permission(perms.FORM_UPDATE))]):
...
@form_router.delete("/{form_id}", status_code=status.HTTP_204_NO_CONTENT,
                    dependencies=[Depends(require_permission(perms.FORM_DELETE))])
