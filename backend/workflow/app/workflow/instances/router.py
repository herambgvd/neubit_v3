...
async def _inst_svc(db: Annotated[AsyncSession, Depends(get_db)], scope: Annotated[Scope, Depends(get_scope)]):
...
async def list_instances(
    ...
    sop_id: Annotated[Optional[str], Query()] = None,
...
async def transition_instance(instance_id: str, body: S.TransitionInstanceRequest,
                              svc: Annotated[InstanceService, Depends(_inst_svc)],
                              actor: Annotated[Principal, Depends(require_permission(perms.INSTANCE_UPDATE))]):
