...
# Import every model module so its tables register on Base.metadata. A model
# Alembic cannot see is not reported as missing — it is proposed for DROP.
# ``app.workflow.tables`` is the single list that has to be complete; keep the
# indirection so there is one place to get wrong instead of two.
