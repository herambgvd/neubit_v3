...
# ── Role succession ──────────────────────────────────────────────────────────
#
# When a gateway rebuild RENAMES a point's tag, the operator's role binding is
# simply stranded on a point that has stopped reporting — which is the state all 19 of this
...
async def repoint_roles(db: Db, scope: Caller, body: RepointRolesRequest) -> dict:
    ...
    same continuity chain a retirement writes, so a rename joins history the
...
async def undo_repoints(db: Db, scope: Caller, body: UndoRepointRequest) -> dict:
    ...
    door; this is the repoint's undo.
...
async def forget_roles(db: Db, scope: Caller, body: ForgetRolesRequest) -> dict:
    ...
    so it takes the ids a human named, the same rule every undo here
