...
- imports the shared font + `@/styles/theme.css` + `@/components/Providers` in `src/app/layout.js`
...
own `src/app/` + `src/components/`, and import shared bits via `@/components/ui/kit`, `@/lib/api`, etc.
