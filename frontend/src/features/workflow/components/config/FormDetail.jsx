  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-nb-faint">Fields</h3>
        {fields.length === 0 ? (
          <p className="text-sm text-nb-faint">No fields defined.</p>
        ) : (
          <ul className="rounded-lg border border-nb-line divide-y divide-nb-line">
            {fields.map((f, i) => (
              <li key={f.id || i} className="flex items-center justify-between gap-3 px-3 py-2.5">
                ...
                <span className="text-[10px] rounded-full bg-[rgba(96,165,250,.1)] px-2 py-0.5 font-medium text-nb-faint uppercase shrink-0">{titleize(f.type)}</span>
              </li>
            ))}
          </ul>
        )}
