  ...
  return (
    <li className="relative">
      ...
      >
        ...
        <span className="relative inline-flex h-9 w-9 items-center justify-center rounded-md shrink-0 text-white border border-nb-line" style={{ background: color }}>
          <Icon icon="heroicons:tag" className="text-base" />
          <span
            className={`absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full border-2 border-nb-line ${
              t.is_active !== false ? "bg-green-500" : "bg-white/5"
            }`}
          />
        </span>
        <span className="flex-1 min-w-0">
          ...
          {t.description && <span className="block text-xs text-nb-muted truncate">{t.description}</span>}
          <span className="block text-[10px] font-mono text-nb-faint truncate">{color.toUpperCase()}</span>
        </span>
      </button>
    </li>
  );
