...
function Section({ icon, title, link, linkLabel, children }) {
  return (
...
function Row({ label, value, tone = "ink", note }) {
...
export default function SystemAssurance() {
  ...
  const features = useQuery(q(["features"], "/features"));
  const policy = useQuery(q(["security-policy"], "/security/policy", { enabled: canSec }));
  const users = useQuery(q(["users", "assurance"], "/auth/users?page_size=100", { enabled: can("user.read") }));
  const dual = useQuery(q(["dual-auth", "pending"], "/security/dual-auth?status=pending&page_size=1", { enabled: canSec }));
  const evidence = useQuery(q(["evidence", "active"], "/vms/evidence?active_only=true&limit=1", { enabled: can("vms.playback.view") }));
  const directory = useQuery(q(["directory"], "/security/directory", { enabled: canSec }));
  const sso = useQuery(q(["sso"], "/security/sso", { enabled: canSec }));
  const settings = useQuery(q(["settings-config"], "/settings", { enabled: can("settings.manage") }));
