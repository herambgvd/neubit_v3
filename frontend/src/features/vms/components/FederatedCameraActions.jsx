...
const READY = new Set(["ready", "done", "complete", "completed", "succeeded"]);
const FAILED = new Set(["failed", "error"]);

export default function FederatedCameraActions({ camera }) {
  const { can } = useAuth();
  ...
  const [recBusy, setRecBusy] = useState(null); // "start" | "stop" | null
...
function FedHoldModal({ camera, onClose }) {
  const node = camera.node_id;
  const cam = camera.real_id;
  const now = new Date();
...
function FedExportModal({ camera, onClose }) {
  const node = camera.node_id;
  const cam = camera.real_id;
  const now = new Date();
