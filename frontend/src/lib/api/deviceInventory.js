...
// gates, fire = panels).
//
// In neubit_v3 the only device backend that exists today is ACCESS-CONTROL
// (cameras/NVR are VMS and still guarded; fire is not built). So this module wires
// ONLY the access source for now — access controllers (instances) + doors — and is
// structured so vms/fire sources drop in later without churn.
//
...
// "/api/v1/access/*" → the access service.
